...
\begin{itemize}
    ...
    \item \texttt{zero\_rectify(value)}: Nếu \texttt{value} đủ nhỏ thì trả về \texttt{0.0} thay vì giữ nguyên giá trị nhiễu số học (ví dụ: \texttt{-2.77e-16}), tránh hiện tượng ``ô nhiễm'' kết quả qua nhiều bước tính toán.
...
Một điểm khác biệt đáng chú ý của dự án này so với dự án trước là sự xuất hiện của \texttt{test\_utils.py} --- một module kiểm thử dùng chung hoàn chỉnh. Module này phục vụ ba mục đích:
...
Sau mỗi phép toán lớn (một bước khử Gauss, một vòng lặp coordinate descent, một lần Gram--Schmidt), kết quả được lọc qua \texttt{zero\_rectify}. Điều này ngăn các số dư như \texttt{-2.77e-16} làm ``ô nhiễm'' các bước tính toán tiếp theo --- đặc biệt quan trọng trong \texttt{build\_null\_vector} khi chiếu vector ra khỏi không gian cột của $\mathbf{X}$.
